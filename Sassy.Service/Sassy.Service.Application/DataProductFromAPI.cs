...
{
    public class DataProductFromSAP : IModule
    {
        private IApplicationService _applicationService;
        private ISAPService _sAPService;
        private appEntities _appDB;
        public DataProductFromSAP(IApplicationService applicationService, ISAPService sAPService, appEntities appEntities)
        {
        ...
        public void Init()
        {
            ...
            {
                //读取配置
                serviceConfig = _applicationService.InitService<DataProductFromSAP>();
        ...
        {
            /***********下载产品数据***************/
            string _msg = DownProductDataFromSAP();
            /***************************************/
        ...
        /// 下载产品数据
        /// </summary>
        private string DownProductDataFromSAP()
        {
            List<string> _msgList = new List<string>();
            FileLogHelper.WriteLog($"Start to download material && sparepart from SAP.", baseModel.ThreadName);
            //保存产品
            var _result = _sAPService.DownMaterial();
            //记录结果
            _msgList.Add($"->File download success:[{string.Join(",", _result.SuccessFiles)}],fail:[{string.Join(",", _result.FailFiles)}]");
            _msgList.Add("Material:");
            //->Material
            _msgList.Add($"->Total Record:{_result.ResultData.Where(p => !string.IsNullOrEmpty(p.Data.SKU)).Count()},Success Record:{_result.ResultData.Where(p => !string.IsNullOrEmpty(p.Data.SKU) && p.Result).Count()},Fail Record:{_result.ResultData.Where(p => !string.IsNullOrEmpty(p.Data.SKU) && !p.Result).Count()}.");
            //->SparePart
            _msgList.Add("SparePart:");
            _msgList.Add($"->Total Record:{_result.ResultData.Where(p => p.Data.SparePartId > 0).Count()},Success Record:{_result.ResultData.Where(p => p.Data.SparePartId > 0 && p.Result).Count()},Fail Record:{_result.ResultData.Where(p => p.Data.SparePartId > 0 && !p.Result).Count()}.");
